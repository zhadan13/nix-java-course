        ...
        <div class="blog-header py-3">
            <nav class="container d-flex align-items-center flex-column flex-md-row justify-content-between">
                ...
                <div class="col-4 d-flex justify-content-end align-items-center">
                    ...
                    <a class="btn btn-sm btn-outline-secondary" href="<c:url value="/index.jsp"/>">Sign in</a>
                </div>
            </nav>
        </div>
...
<div class="container mt-5">
    ...
    <div class="alert alert-info mt-2">
        <c:choose>
            <c:when test="${sessionScope.posts == null || sessionScope.posts.size() == 0}">
                <p>So far it's empty.</p>
            </c:when>
            <c:otherwise>
                <c:forEach var="post" items="${sessionScope.posts}">
                    <h3><c:out value="${post.title}"/></h3>
                    <p><c:out value="${post.date}"/></p>
                    <p><c:out value="${post.text}"/></p>
                    <a href="'/blog/' + ${post.id}" class="btn btn-success">Details</a>
                </c:forEach>
            </c:otherwise>
        </c:choose>
    </div>
